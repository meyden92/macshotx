...
- **Select-tool click precedence, first match wins:** hits an annotation → select it; a selected set or an active text edit exists → clear it and capture nothing; a window is under the cursor with snap armed → capture it; otherwise → capture the display under the cursor.
